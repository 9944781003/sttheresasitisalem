...
class CSS extends Minify
{
    ...
    protected $importExtensions = array(
      /*  'gif' => 'data:image/gif',
        ...
        'ttf' => 'data:application/x-font-truetype',
        'otf' => 'data:application/x-font-opentype',
        'tif' => 'image/tiff',
        'tiff' => 'image/tiff',
        'xbm' => 'image/x-xbitmap',*/
    );
    ...
    {   if(!empty($this->importExtensions)){
    ...
    public function execute($path = null)
    {
        ...
        foreach ($this->data as $source => $css) {
            ...
            $converter = new Converter($source, $path ?$path: $source);
            $css = $this->move($converter, $css);

            // if no target path is given, relative paths were not converted, so
            // they'll still be relative to the source file then
            $css = $this->importFiles($path ?$path: $source, $css);
            $css = $this->combineImports($path ?$path: $source, $css);
