...
class Themify {
	...
	function template_redirect() {
		...
		if ( is_page() || themify_is_shop() ) {
			...
			} elseif ( is_page() && themify_check( 'setting-default_page_layout' ) ) {
				$this->layout = themify_get( 'setting-default_page_layout','',true );
			} elseif ( themify_is_shop() && themify_check( 'setting-shop_layout' ) ) {
				$this->layout = themify_get( 'setting-shop_layout','',true );
			}else {
		...
		elseif ( is_post_type_archive( 'portfolio' ) || is_tax('portfolio-category') ) {
			...
			$this->hide_date = themify_get( 'setting-default_portfolio_index_hide_post_date', 'yes' );
		...
		elseif ( is_single() ) {
			...
			if ( !in_array($postType, $excluded_types) ) {
				...
				$this->layout = in_array( themify_get( $layout ), $sidebar_mode )  ? themify_get( $layout ) : themify_get( 'setting-'.$layout );
			}
		...
		if(!is_search() && !is_singular( array('post', 'portfolio') )) {

			if(themify_is_shop()){
				$this->post_layout_type = themify_get( 'setting-product_content_layout','' );
			}else{
				$this->post_layout_type = themify_get($this->query_post_type . '_content_layout', 'default') === 'default'
    ...
    private function themify_get_sticky_slider_value(){
		...
		if ( is_page() ) {
		...
		} elseif ( themify_is_woocommerce_active() && ( is_product_category() || is_product_tag() ) ) {
			$option = 'setting-shop-archive_sticky_sidebar';
			...
			$option = 'setting-single_product_sticky_sidebar';
		}

		if($option!==null){
			$value = (bool) themify_get( $option );
			if ( is_singular() ) {
				$value = (bool) themify_get( 'post_sticky_sidebar', $value );
			}
		}
		else{
			$value = false;
