...
/**
 * Returns the URL to Fontello stylesheet
 *
 * @return string|false
 */
function themify_fontello_get_stylesheet() {
	$path = themify_fontello_path();
	if ( ! $path ) {
		return false;
	}

	if ( ! file_exists( $path['dir'] . 'css/themify-fontello.css' ) ) {
		$prefix = themify_fontello_get_config( 'css_prefix_text', 'icon-' );
		$stylesheet = file_get_contents( $path['dir'] . 'css/' . themify_fontello_get_config( 'name', 'fontello' ) . '-embedded.css' );
		// replace the prefix for icons
		$stylesheet = preg_replace( '/'. $prefix .'/', 'tf_fontello-', $stylesheet );
		file_put_contents( $path['dir'] . 'css/themify-fontello.css', $stylesheet );
	}

	if ( file_exists( $path['dir'] . 'css/themify-fontello.css' ) ) {
		return $path['url'] . 'css/themify-fontello.css';
	} else {
		return false;
	}
}
...
function themify_enqueue_fontello() {
	if( themify_check( 'setting-fontello' ) && $stylesheet = themify_fontello_get_stylesheet() ) {
		themify_enque_style( 'themify-fontello', $stylesheet );
	}
}
...
function themify_fontello_path() {
	...
	if( is_dir( $dest . $base ) ) {
		...
		$path = themify_get_absolute_path( $path );
		if ( $path === false ) {
			// could not get the file path, abort.
			return false;
		}
		WP_Filesystem();
	...
	if( is_wp_error( $result ) || !is_dir( $dest . $base ) ) {
		return false;
	} elseif( 1 == $result ) {
		return array(
