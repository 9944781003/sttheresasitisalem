...
class TFCache {
    ...
    /**
     * Check whether caching feature can be enabled
     * 
    ...
    public static function check_requirements() {
        ...
        if ( is_null( $v ) ) {
            if ( ! version_compare( PHP_VERSION, '5.4', '>=' ) ) {
				$v = new WP_Error( 'php_version', __( 'This feature requires PHP 5.4 or higher. Please contact your website administrator.', 'themify' ) );
			} else if ( defined( 'WP_ROCKET_VERSION' ) ) {
				$v = new WP_Error( 'caching_plugin', __( 'WP Rocket plugin is active. This feature is disabled.', 'themify' ) );
			} else {
				$v = true;
			}
        }
...
if (!defined('THEMIFY_BUILDER_VERSION') && ! is_wp_error( TFCache::check_requirements() ) && TFCache::is_cache_activate() && (!isset($_POST['action']) || (isset($_POST['action']) && $_POST['action']==='themify_clear_all_caches')) && !isset($_GET['builder_grid_activate'])) {  
