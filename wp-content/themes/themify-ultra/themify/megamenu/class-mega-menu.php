...
class Themify_Widgets_Menu {
	...
	function nav_menu_script() {
		...
		wp_enqueue_script( 'themify-widgets-menu-admin', themify_enque(THEMIFY_URI . '/megamenu/js/admin-nav-menu.js'), array( 'jquery' ),THEMIFY_VERSION,true );
	...
	function get_widget_menu_class( $item ) {
		$type = ltrim( $item->url, '#' );
		if( in_array( $type, $this->get_widget_types(),true ) ) {
			return $type;
		}
